"""
Централизованная система конфигурации для модуля preprocessing.
...
def load_env_file(env_file: Optional[Path] = None) -> None:
    ...
    Args:
        env_file: Путь к .env файлу. Если не указан, ищет в корне проекта.
    """
    if env_file is None:
...
class SyncDBConfig:
    ...
    local_mongo: MongoConfig = field(default_factory=lambda: MongoConfig(
        server=os.environ.get("LOCAL_MONGO_SERVER", "localhost:27017"),
