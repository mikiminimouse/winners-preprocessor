    ...
    try:
        ...
        if local_result.is_healthy():
            ...
            if local_result.details:
                ...
                print("❌ Локальная Metadata MongoDB: ошибка подключения")
            except ServerSelectionTimeoutError:
                print("❌ Локальная Metadata MongoDB: таймаут подключения")
            except Exception as e:
                print(f"❌ Локальная Metadata MongoDB: {str(e)[:50]}...")
        else:
            print("⚠️  Локальная Metadata MongoDB: не настроена (нет учетных данных)")
