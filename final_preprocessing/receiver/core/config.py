...
def load_env_file(env_file: Optional[Path] = None) -> None:
    ...
    if env_file is None:
        ...
        project_root = Path(__file__).parent.parent.parent.parent  # Теперь на уровень выше (final_preprocessing -> winners_preprocessor)
...
class SyncDBConfig:
    ...
    remote_mongo: MongoConfig = field(default_factory=lambda: MongoConfig(
        ...
        ssl_cert=os.environ.get("MONGO_SSL_CERT", "/root/winners_preprocessor/receiver/certs/sber2.crt"),
