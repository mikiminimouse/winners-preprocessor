...
def export_to_mongodb(
    ...
    Args:
        ...
        contract: Contract из docprep (docprep.contract.json)
    ...
    try:
        ...
        mongo_doc = {
            "unit_id": unit_id,
            "protocol_id": "",  # Contract не содержит protocol_id
            "protocol_date": unit_info.get("batch_date", ""),
            "route": routing.get("docling_route", "unknown"),
            "docling_document": doc_dict,
            "export_formats": ["mongodb"],
            "created_at": datetime.utcnow(),
            "contract": contract,  # Полный contract для контекста
            "cost_estimation": contract.get("cost_estimation", {}),
        }
