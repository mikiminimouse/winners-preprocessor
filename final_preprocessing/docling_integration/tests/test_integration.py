...
# Теперь docling_integration находится в final_preprocessing/docling_integration
...
from docling_integration.bridge_docprep import load_unit_from_ready2docling, get_main_file
from docling_integration.config import build_docling_options, get_input_format_from_route


def test_unit_loading(unit_path: Path) -> None:
    ...
    try:
        ...
        assert unit_data['unit_id'] is not None
        assert unit_data['route'] is not None
        
        # Проверяем contract (используется ТОЛЬКО contract, не manifest)
        contract = unit_data.get("contract", {})
        routing = contract.get("routing", {}).get("docling_route")
        
        logger.info(f"  - Route from contract: {routing}")
        
    except Exception as e:
        logger.error(f"❌ Failed to load unit: {e}", exc_info=True)
        pytest.fail(f"Failed to load unit: {e}")


def test_config_mapping(route: str, contract: Optional[Dict[str, Any]] = None) -> None:
    """Тестирует маппинг route → Docling options."""
    logger.info(f"Testing config mapping for route: {route}")
    
    try:
        # build_docling_options теперь использует только route (manifest не нужен)
        ...
        if options:
            ...
            pytest.skip("Docling options not available")
            
    except Exception as e:
        logger.error(f"❌ Failed to build options: {e}", exc_info=True)
        pytest.fail(f"Failed to build options: {e}")


def test_main_file_detection(unit_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if main_file:
            ...
            if main_file.exists():
                logger.info(f"  - Size: {main_file.stat().st_size} bytes")
                
        else:
            logger.warning("⚠️  No main file found")
            # Если файла нет, это может быть ошибкой или нет, зависит от теста
            # Но для теста мы ожидаем, что файл будет найден
            pytest.fail("No main file found in unit data")
...
def test_pipeline_processing(unit_path: Path, limit: int = 3) -> None:
    ...
    try:
        ...
        if result["success"]:
            ...
            pytest.fail(f"Pipeline processing failed: {result.get('errors', [])}")
            
    except Exception as e:
        logger.error(f"❌ Pipeline processing failed: {e}", exc_info=True)
        pytest.fail(f"Pipeline processing failed with exception: {e}")
...
def main():
    ...
    for unit_path in unit_paths:
        ...
        # Тест 3: Маппинг route → options (contract не нужен, используется только route)
        config_result = test_config_mapping(route)
